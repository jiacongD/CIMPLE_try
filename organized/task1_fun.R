...
standard_LME_fun = function(long_data){
...
JMVL_LY_fun = function(long_data){
...
IIRR_weighting_fun = function(long_data){
...
JMVL_Liang_fun = function(long_data){  
    data = long_data
...
imputation_LME_fun = function(long_data,imp_time_factor=NULL){
    ...
    if(is.null(imp_time_factor)){
        ...
        data_base = data %>% dplyr::select(-Y,-time) %>% unique()
        data_full = data_base %>%
...
JMVL_G_fun = function(long_data){    
    ...
    check_points = list(Y_mat = Y_mat,
                        S_mat = S_mat,
                        B_mat = B_mat,
                        Rho_mat = Rho_mat)
    estimates = list(betas = betas,
                    sigma2_e = sigma2_e,
                    gammas = gammas,
                    sigma2_b = sigma2_b,
                    rho = rho)
