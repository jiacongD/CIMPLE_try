...
JMLD_fun = function(long_data, surv_data){
...
VA_JMLD_fun = function(long_data, surv_data){
  # remove patients with no Y measurement
  zeroRecords_id = long_data[is.na(long_data$Y),"id"]
  long_data = long_data[!long_data$id %in% zeroRecords_id,]
  surv_data = surv_data[!surv_data$id %in% zeroRecords_id,]  
...
Imputation_Cox_fun = function(long_data,surv_data,imp_time_factor=NULL){
    data = long_data
    ...
    imp1=mice(as.matrix(df_full), m=5, 
...
VAImputation_Cox_fun = function(long_data,surv_data,imp_time_factor=NULL){
    data = long_data
    ...
    df_full = df_full%>% as.data.frame()%>% dplyr::select(all_of(c(colnames(data),"Ni")))
    head(df_full)
